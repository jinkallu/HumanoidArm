...
def qpos_from_site_pose(physics,
  ...
  joint_range = np.copy(physics.model.jnt_range) # joints limits 
  # sanity check to see if the local limits are in the range
  # also set the local values for IK calculation
  if joint_loc_lmts is not None:
    for loc_lmt in joint_loc_lmts:
      joint_name = loc_lmt["joint_name"]
      loc_upper_lmt = loc_lmt["upper_lmt"]
      loc_lower_lmt = loc_lmt["lower_lmt"]

      joint_idx = physics.model.name2id(joint_name, "joint") 
      if loc_lower_lmt < joint_range[joint_idx][0]:
        print("Error! Local lower limit {loc_lower_lmt} for joint {joint_name} is too low")
        return
      
      if loc_upper_lmt > joint_range[joint_idx][1]:
        print("Error! Local upper limit {loc_upper_lmt} for joint {joint_name} is too high")
        return
      
      joint_range[joint_idx][0] =  loc_lower_lmt
      joint_range[joint_idx][1] =  loc_upper_lmt
  ...
  for steps in range(max_steps):
    ...
    if target_pos is not None:
      ...
      for joint_name in joint_names:
        joint_idx = physics.model.name2id(joint_name, "joint")
        qpos_i = physics.named.data.qpos[joint_name]
        # if qpos_i < joint_range[joint_idx][0] or qpos_i > joint_range[joint_idx][1]:
        #     err_norm += 1
        if qpos_i < joint_range[joint_idx][0]:
          physics.named.data.qpos[joint_name] = joint_range[joint_idx][0]; 
        elif qpos_i > joint_range[joint_idx][1]:
          physics.named.data.qpos[joint_name] = joint_range[joint_idx][1]; 
    ...
    if target_quat is not None:
      ...
      #print("Rot error",np.linalg.norm(err_rot)*rot_weight)
